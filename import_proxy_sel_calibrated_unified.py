#!/usr/bin/env python3
"""
Unified Parquet Import Tool for proxy_sel_calibrated Database
Imports calibrated account_voted and account_unvoted parquet files into the proxy_sel_calibrated database
"""
...
def get_db_connection():
    """Create database connection to proxy_sel_calibrated with fallback options"""
    try:
        print("🔍 Connecting to MySQL proxy_sel_calibrated database...")
...
def validate_and_clean_data(df, table_name):
    ...
    # Define expected columns for proxy_sel_calibrated tables
    expected_columns = [
    ...
    if missing_columns:
        ...
        # Common variations to check for calibrated data
        column_variations = {
            'score_model2': ['score_model1', 'score', 'score_model', 'calibrated_score'],
            'prediction_model2': ['prediction_model1', 'prediction', 'prediction_model', 'calibrated_prediction'],
...
def import_parquet_to_table(connection, parquet_file, table_name):
    ...
    try:
        ...
        print(f"📥 Loading calibrated parquet file: {parquet_file}")
        ...
        print(f"📊 Importing {len(df)} calibrated records...")
        ...
        for i in range(0, len(df), batch_size):
            ...
            progress_pct = (imported_count/len(df)*100)
            print(f"📈 Progress: {imported_count}/{len(df)} records ({progress_pct:.1f}%)")
        
        print(f"✅ Successfully imported {imported_count} calibrated records to {table_name} table")
...
def main():
    """Main function"""
    print("=== Unified Parquet Import Tool for proxy_sel_calibrated Database ===")
    print("🎯 Importing calibrated account_voted and account_unvoted parquet files")
    print("")
    
    # Default calibrated parquet file paths
    default_files = {
    ...
    if len(sys.argv) >= 3:
        ...
        print(f"📁 Using default calibrated file paths:")
        print(f"   Unvoted: {unvoted_file}")
        print(f"   Voted: {voted_file}")
    ...
    if os.path.exists(unvoted_file):
        ...
        print(f"✅ Found calibrated unvoted file: {unvoted_file}")
    else:
        print(f"❌ Calibrated unvoted file not found: {unvoted_file}")
    
    if os.path.exists(voted_file):
        files_to_process.append((voted_file, 'account_voted'))
        print(f"✅ Found calibrated voted file: {voted_file}")
    else:
        print(f"❌ Calibrated voted file not found: {voted_file}")
    
    if not files_to_process:
        print("❌ No calibrated parquet files found to process")
        print("")
        print("Expected files:")
        print(f"  - {unvoted_file}")
        print(f"  - {voted_file}")
        print("")
        print("Usage:")
        print("  python3 import_proxy_sel_calibrated_unified.py")
        print("  python3 import_proxy_sel_calibrated_unified.py <unvoted_file.parquet> <voted_file.parquet>")
        sys.exit(1)
    ...
    try:
        ...
        for file_path, table_name in files_to_process:
            ...
            print(f"🧪 Calibrated data import for proxy_sel_calibrated database")
            print(f"{'='*70}")
        ...
        # Final verification
        print(f"{'='*70}")
        print("🔍 FINAL VERIFICATION - CALIBRATED DATA")
        print(f"{'='*70}")
        ...
        print(f"📊 Final calibrated database statistics:")
        ...
        if total_imported > 0:
            print("🎉 Calibrated data import completed successfully!")
            ...
            print("❌ Calibrated data import failed or no records imported")
            sys.exit(1)
        
        cursor.close()
        
    except Exception as e:
        print(f"❌ Error during calibrated data import: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
    finally:
