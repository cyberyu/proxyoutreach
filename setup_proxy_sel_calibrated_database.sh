...
# Setup proxy_sel_calibrated2 database with proper schema for account tables

set -e

echo "🔧 Setting up proxy_sel_calibrated2 database..."
...
echo "🏗️ Creating proxy_sel_calibrated2 database and setting up users..."
"${MYSQL_CMD[@]}" << EOF
-- Create proxy_sel_calibrated2 database
CREATE DATABASE IF NOT EXISTS proxy_sel_calibrated2;
...
-- Grant privileges for proxy_sel_calibrated2 database
GRANT ALL PRIVILEGES ON proxy_sel_calibrated2.* TO 'webapp'@'localhost';
GRANT ALL PRIVILEGES ON proxy_sel_calibrated2.* TO 'webapp'@'%';

FLUSH PRIVILEGES;

-- Use the new database
USE proxy_sel_calibrated2;
...
echo "✅ proxy_sel_calibrated2 database and tables created successfully!"
echo "📊 Database: proxy_sel_calibrated2"
echo "📋 Tables created:"
echo "   - account_unvoted (with indexes)"
echo "   - account_voted (with indexes)"
echo "👤 User: webapp (with full privileges)"
echo ""
echo "🎯 Ready for calibrated parquet file import!"
