...
# Setup proxy_sds_calibrated database with proper schema for SDS calibrated account tables

set -e

echo "🔧 Setting up proxy_sds_calibrated database..."
...
echo "🏗️ Creating proxy_sds_calibrated database and setting up users..."
"${MYSQL_CMD[@]}" << EOF
-- Create proxy_sds_calibrated database
...
-- Create account_unvoted table for SDS calibrated data
CREATE TABLE IF NOT EXISTS account_unvoted (
    ...
    INDEX idx_score_model2 (score_model2),
    INDEX idx_prediction_model2 (prediction_model2)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Create account_voted table for SDS calibrated data
...
echo "✅ proxy_sds_calibrated database and tables created successfully!"
echo "📊 Database: proxy_sds_calibrated"
echo "📋 Tables created:"
echo "   - account_unvoted (with enhanced indexes for SDS calibrated data)"
echo "   - account_voted (with enhanced indexes for SDS calibrated data)"
echo "👤 User: webapp (with full privileges)"
echo ""
echo "🎯 Ready for SDS calibrated parquet file import!"
